...
#define SLD_STENCIL3D_TRY 0x052
#define DEV_NAME "sld,stencil3d_try"
...
int main(int argc, char * argv[])
{
	...
	ndev = probe(&espdevs, SLD_STENCIL3D_TRY, DEV_NAME);
	if (ndev == 0) {
#ifndef __riscv
		printf("stencil3d_try not found\n");
#else
		print_uart("stencil3d_try not found\n");
#endif
		return 0;
	}
	...
	for (n = 0; n < ndev; n++) {
		...
		/* <<--regs-config-->> */
		iowrite32(dev, STENCIL3D_TRY_ROW_SIZE_REG, row_size);
		iowrite32(dev, STENCIL3D_TRY_HEIGHT_SIZE_REG, height_size);
		iowrite32(dev, STENCIL3D_TRY_COL_SIZE_REG, col_size);
