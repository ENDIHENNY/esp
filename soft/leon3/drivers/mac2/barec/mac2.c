...
#define SLD_MAC2 0x152
#define DEV_NAME "sld,mac2"
...
int main(int argc, char * argv[])
{
	...
	ndev = probe(&espdevs, SLD_MAC2, DEV_NAME);
	if (ndev == 0) {
#ifndef __riscv
		printf("mac2 not found\n");
#else
		print_uart("mac2 not found\n");
#endif
		return 0;
	}
	...
	for (n = 0; n < ndev; n++) {
		...
		/* <<--regs-config-->> */
		iowrite32(dev, MAC2_MAC_N_REG, mac_n);
		iowrite32(dev, MAC2_MAC_VEC_REG, mac_vec);
		iowrite32(dev, MAC2_MAC_LEN_REG, mac_len);
