...
void stencil_compute(int plm_adj, int32_t C0, int32_t C1, int32_t row_size, int32_t col_size, int32_t height_size, sc_dt::sc_int<DATA_WIDTH> orig[PLM_IN_WORD], sc_dt::sc_int<DATA_WIDTH> sol[PLM_OUT_WORD]) {
    ...
    if (PLM_IN_WORD < row_size * col_size * height_size) {
		...
		int abs_idx_h1 = abs_idx - (map * (height_size - 1));


		// -> Height Boundary
		if ( abs_idx >= 0 && abs_idx <= map - 1)	{
			sol[index0] = orig[index0];
			continue;
		}
		if ( abs_idx_h1 >= 0 && abs_idx_h1 <= map - 1)	{
			sol[index0] = orig[index0];
			continue;
		}

		// -> Column Boundary
		int abs_idx_c1 = abs_idx - (map - row_size);
		if ( ((abs_idx % map) >= 0 &&  (abs_idx % map) < row_size) && ((abs_idx / map) >= 1 && (abs_idx / map) < height_size - 1))	{
			sol[index0] = orig[index0];
			continue;
		}
		if ( ((abs_idx_c1 % map) >= 0 &&  (abs_idx_c1 % map) < row_size) && ((abs_idx_c1 / map) >= 1 && (abs_idx_c1 / map) < height_size - 1))	{
			sol[index0] = orig[index0];
			continue;
		}

		// -> Row Boundary
		int temp = (abs_idx % map) % row_size;
		int temp1 = abs_idx / map;
		int temp2 = (abs_idx % map) / row_size;
		if (temp == 0 && temp1 >= 1 && temp1 < height_size - 1 && temp2 >= 1 && temp2 < col_size - 1)	{
			sol[index0] = orig[index0];
			continue;
		}

		int abs_idx_r1 = abs_idx - (row_size - 1);
		temp = (abs_idx_r1 % map) % row_size;
		temp1 = abs_idx_r1 / map;
		temp2 = (abs_idx_r1 % map) / row_size;
		if (temp == 0 && temp1 >= 1 && temp1 < height_size - 1 && temp2 >= 1 && temp2 < col_size - 1)	{
			sol[index0] = orig[index0];
			continue;
		}
